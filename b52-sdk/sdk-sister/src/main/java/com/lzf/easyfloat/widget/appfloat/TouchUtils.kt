...
internal class TouchUtils(val context: Context, val config: FloatConfig) {
    ...
    private fun sideAnim(
            ...
            SidePattern.RESULT_SIDE -> {
                if (minX < minY) {
                    ...
                    if (leftDistance < rightDistance) {
                        EasyFloat.lastFlag = 1
                        0
                    } else {
                        EasyFloat.lastFlag = 3
                        params.x + rightDistance
                    }
                } else {
                    isX = false
                    if (topDistance < bottomDistance) {
                        EasyFloat.lastFlag = 2
                        0
                    } else {
                        EasyFloat.lastFlag = 4
                        if (hasStatusBar) {
                            emptyHeight - statusBarHeight(view)
                        } else {
                            emptyHeight
                        }
                    }
        ...
        EasyFloat.offsetSlideEnd = offsetSlideEnd(view)// hack

        val animator = ValueAnimator.ofInt(if (isX) params.x else params.y, end + EasyFloat.offsetSlideEnd)
    ...
    private fun offsetSlideEnd(view: View): Int {
        var xWidth = view.measuredWidth
        var xHeight = view.measuredHeight
        return when (EasyFloat.lastFlag) {
            1 -> 0 - xWidth / 2
            2 -> 0 - xHeight / 2
            3 -> xWidth / 2
            4 -> xHeight / 2
